...
// 텍스트 기반의 통신을 구현해볼 것 이므로 'TextWebSocketHandler'를 상속받아서 작성한다.
	...
	//1. 새로운 클라이언트와 소켓이 연결되면 ★★★모든 접속자★★★들에게 'id ... 님이 접속하였습니다.' 라고 보낸다.
	private List<WebSocketSession> wsslist = new ArrayList<>();		//1-a.접속자들의 세션을 저장할 리스트

	//2. 좋아요를 클릭하면 ★★★글 주인에게만★★★ 메시지 보내기
	private Map<String,WebSocketSession> wsmap = new HashMap<>();

	//좋아요에 필요한 서비스 클래스 자동 주입 (생성자 + setter)
	private GalleryService service;

	@Autowired	//socket config 클래스에서 기본 생성자를 사용중이므로 setter 주입으로 한다.
	public void setService(GalleryService service) {
		this.service = service;
	}

	//소켓 연결이 이루어 진 후에 할일 처리 - 새로운 클라이언트와의 연결이 발생하면 실행되는 메소드
	@Override   //지금까지 세션은 http프로토콜 세션(HttpSession) , 웹 소켓은 ws프로토콜 , 전달받는 세션은 소켓세션
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		//데이터를 수신하면 처리하는 핸들러 메소드 (session : 누가 보냈는지 , message : 보낸 내용)
		...
		if(content.startsWith("open")) {			//2-e. 변수명을 이해하기 쉬운 이름으로 변경(받은 메시지 변수)
			...
			//map 에 사용자 id 를 key , 소켓 세션을 value 로 저장하기 - 좋아요에서 필요
			wsmap.put(senderId,session);
		} //if end

		if(content.startsWith("close")) {			//2-e. 변수명을 이해하기 쉬운 이름으로 변경(받은 메시지 변수)
			senderId = content.substring(6);		//  클라이언트가 보낸 open/접속자id 에서 / 기호 뒤의 접속자 id만 가져오기
			for(WebSocketSession ws : wsslist)		// 모든 접속자에게 보내기
				// ws 에게 메시지 보내기 : 보낼 메시지 타입은 TextMessage
